...
jasmine.executeSpecsInFolder = function(options){
  ...
  if(junitreport && junitreport.report) {
    ...
    var jreporter = new jasmine.JUnitXmlReporter(junitreport.savePath,
                                                        junitreport.consolidate,
                                                        junitreport.useDotNotation)

    try {
      var patch = require('./write-file-patch.js');

      // overwrite JUnitXmlReporter.writeFile with our patched version
      jreporter.writeFile = patch.writeFile;

      DEBUG && console.log('patched JUnitXmlReporter.writeFile');
    } catch (ex) {
      console.log(ex);
    }
    
    jasmineEnv.addReporter(jreporter);
  }
